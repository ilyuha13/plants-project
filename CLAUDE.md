...
1. **Показывать статистику использования (в начале каждой сессии):**
   ```
   📊 Статистика сессии:
   - Токены: X,XXX / 200,000 (XX% использовано)
   - Осталось: XXX,XXX токенов
   - Статус: 🟢 Достаточно | 🟡 Осторожно (<30%) | 🔴 Мало (<10%)
   ```

   **Примечание:** Это бюджет токенов для текущего разговора.
   Для просмотра лимитов Claude Pro плана → Settings в claude.ai

2. **Объяснять все команды и изменения:**
...
5. **Управление контекстом:**
   - При приближении к лимиту токенов (<30%) - предупреждать
   - Предлагать завершить текущую задачу и начать новую сессию
   - Избегать избыточного вывода при малом остатке токенов
