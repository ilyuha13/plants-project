...
import { LifeFormDetailRouteParams } from '../../lib/routes'
import { trpc } from '../../lib/trpc'

export const EditLifeFormPage = () => {
  const editLifeForm = trpc.editLifeForm.useMutation()
  ...
  const {
    ...
    onSubmit: async (values) => {
      await editLifeForm.mutateAsync(values)
    },
    successMessage: 'жизненная форма обновлена',
    showValidationAlert: true,
    redirectOnSuccess: `/lifeForm/${lifeFormId}`,
  })
  ...
  const initialisation = async () => {
    await formik.setValues({
      id: lifeFormId,
      name: data.lifeForm.name,
      description: data.lifeForm.description || '',
      imagesUrl: data.lifeForm.imagesUrl,
    })
  }
