...
import { zAddLifeFormTrpcInput } from '@plants-project/backend/src/router/addLifeForm/input'
...
export const AddLifeFormPage = () => {
  const addLifeForm = trpc.addLifeForm.useMutation()
  ...
  const {
    ...
    validationSchema: zAddLifeFormTrpcInput,
    onSubmit: async (values) => {
      await addLifeForm.mutateAsync(values)
    },
        ...
        <Grid container spacing={{ xs: 2, md: 4 }}>
          <Grid size={{ xs: 12, md: 6 }}>
            <Stack component="form" onSubmit={formik.handleSubmit} sx={{ p: 2 }}>
              <Typography variant="h2">Добавить жизненную форму</Typography>
